...
> 660/663). The bitset arc moved it twice: MCDC-012's gap closure at
> CI #1253 (1774/2004 → 1787/2004, thirteen outcomes driven, denominator
> unchanged) and VERIFY-020 F2's guard widening at CI #1265
> (1787/2004 → 1795/2012, +8 outcomes and +8 hit). The earlier note read:
> last moved by the deque arc at CI #1240
...
first documented trusted stdio axioms; its residual *classes* were
predicted before the pinning run, with the inherited surface named
exactly (the contract.h pair, in the report-only job at 197ef8e) and
the own surface predicted at class granularity only (report-only
chronology: d8566d5/#1132 at 300s, 1597a51/#1133 name-stable at 120s).
**Corrected 2026-08-09:** an earlier revision of this sentence claimed
the 10-goal set was "predicted by name … with zero goals outside the
documented set". The committed pre-run artifact does not support that:
its header says "PREDICTED shape (to confirm against the first run, not
to assert)", its OWN_CHECKS array was empty, and VERIFY-017 records
that a predicted class (stdio-intrinsic residuals) produced zero goals.
Of the 10 pinned residuals, 2 were named in advance and 8 were not. As of 2026-07-12 (baseline at
...
| 2026-09-05 | d0b6f2c | #1280  | v1.3.0  | 96.7%  | 99.6%     | 89.3%    | 89.9%  | priority_queue coverage arc + **PQ-A** (MCDC-014). `data/priority_queue.h` 62/78 → 82/82 by twelve test gaps — including the large-element swap path, which had **zero executions** and was first misread as a justification row before being corrected — then 82/82 → 81/82 when PQ-A renamed the six heap helpers internal and made the self-swap leg dead by construction (J1). Aggregate 1796/2014 → 1816/2018 → **1815/2018**. PQ-A's exposure was demonstrated, not argued: a client `pq_swap(&q, 0, 4)` broke the heap invariant silently and left an outstanding borrow validating against changed contents — failure-open, the VERIFY-021 mode. |
